...
def kembalian():
    ...
    if uang >= total:
        ...
        messagebox.showinfo(message='Kembalian sebesar {}'.format(str(kembalian)))
    else:
        messagebox.showerror(message='Maaf Uang Yang Diberikan Kurang')
def clear():
        pensil.set('0')
        buku.set('0')
        penghapus.set('0')
...
app.configure(bg='#0000FF') # membuat background warna
...
# footer text
Label(app, text='Created by Rafi', bg='#31c6d4', foreground='#fef5ac', font='arial 10 ').place(x=300,y=550)
